...
def main():
    ...
    parser.add_argument('--local_rank', type=int, help='node rank for distributed parallel')
    ...
    network = utils.MultiCropWrapper(network, iBOTHead(
        in_dim=network.embed_dim,
        nlayers=3,
        bottleneck_dim=args.bottleneck_dim,
    ))
    print(network)
    network.cuda(args.local_rank)
    # utils.load_pretrained_weights(network, pretrained_path, args.checkpoint_key, 'teacher', args.patch_size)
    try:
    ...
    if num_per_cluster is None and args.local_rank == 0:
        ...
        all_dim = torch.arange(args.bottleneck_dim).chunk(args.chunks)
