...
def eval_knn(args):
    ...
    if utils.get_rank() == 0:
        logfile = open(os.path.join(os.path.dirname(args.pretrained_weights), f'knn_{args.train_percent}_percent.txt'), 'a+')
        logfile.writelines(f"--- Results for {args.pretrained_weights} ---\n")
        if args.use_cuda:
        ...
        for k in args.nb_knn:
            ...
            print(f"{k}-NN classifier: Top1: {top1}, Top5: {top5}")
            logfile.writelines(f"{k}-NN classifier: Top1: {top1}, Top5: {top5}\n")
