...
def get_args_parser():
    ...
    parser.add_argument("--train_percent", default=1, type=int, help="only useful when finetune all the projection layers")
...
def build_dataset(is_train, args):
    ...
    if args.data_set == 'CIFAR':
        ...
        if is_train and args.train_files is not None:
            dataset.samples = []
            for fname in args.train_files:
                fname = fname.decode().strip()
                cls = fname.split("_")[0]
                dataset.samples.append(
                    (os.path.join(root, cls, fname), dataset.class_to_idx[cls])
                )
...
def main(args):
    ...
    if utils.is_main_process():
        with open(os.path.join(os.path.dirname(args.pretrained_weights), 'semi_sup_finetune.txt'), 'a+') as f:
            f.writelines(f'Semi supervised finetune: {args.pretrained_weights}\n')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (test_stats["acc1"] >= max_accuracy):
            ...
            if utils.is_main_process():
                with open(os.path.join(os.path.dirname(args.pretrained_weights), 'semi_sup_finetune.txt'), 'a+') as f:
                    f.writelines(f'Max accuracy: {test_stats["acc1"]}%\n')
