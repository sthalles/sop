...
def eval_logistic_regression(args):
    ...
    if utils.get_rank() == 0:
        ...
        f = open(os.path.join(os.path.dirname(args.pretrained_weights), f'logreg_{args.train_percent}.txt'), 'a+')
        for penalty in penalties:
            for solver in solvers:
                for lambd in args.lr_lambd:
                    f.writelines(f"penalty: {penalty}\n")
                    f.writelines(f"solver: {solver}\n")
                    f.writelines(f"lambd: {lambd}\n")
                    acc = logistic_regression(train_features, train_labels,
                        test_features, test_labels, lambd, solver, penalty, args)
...
def extract_feature_pipeline(args):
    ...
    if args.train_percent in {1, 10}:
        ...
        for fname in args.train_files:
            ...
            cls = fname.split("_")[0]
            dataset_train.samples.append(
                (os.path.join(traindir, cls, fname), dataset_train.class_to_idx[cls])
            )
...
def logistic_regression(train_features, train_labels, test_features, test_labels, lambd, solver, penalty, args):
    ...
    # classifier=Classifier(loss='multiclass-logistic',penalty='l1l2',lambda_1=0.0001,max_iter=args.epochs,tol=1e-3,duality_gap_interval=5, verbose=True, fit_intercept=False)
    classifier = LogisticRegression(lambda_1=lambd, solver="catalyst-miso", max_iter=args.epochs, penalty=penalty, tol=1e-8, verbose=True)
    ...
    score = classifier.score(scaler.transform(test_features), test_labels)
    return score
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_lambd', default=[0.00001], nargs='+', type=float,
    ...
    parser.add_argument('--epochs', default=500, type=int, help='Training epochs of logistic regression.')
