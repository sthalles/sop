...
def eval_linear(args):
    ...
    args.lrs = [0.0001, 0.0002, 0.0005, 0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.3, 0.5] # from dinov2
    ...
    if utils.is_main_process():
        with open(os.path.join(os.path.dirname(args.pretrained_weights), 'linear', 'linear_multi.txt'), 'a') as file:
            file.writelines(f"------- Linear evaluation for: {os.path.basename(args.pretrained_weights)} ---------")
    ...
    if utils.is_main_process():
        with open(os.path.join(os.path.dirname(args.pretrained_weights), 'linear', 'linear_multi.txt'), 'a') as file:
            file.writelines(f'Top-1 test accuracy: {best_acc}%\n')
            file.writelines(f'Top-1 test accuracy with sweeping lr only: {best_acc_sweep_lr_only}%\n')
            file.writelines(f"Optim configs with top-1 test accuracy: lr {lr}, wd {wd}, optim {optim}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sweep_lr_only', default=False, type=bool, help='Wether or not to only sweep over learning rate')
    args = parser.parse_args()
    # if args.output_dir:
    #     Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    
    Path(os.path.join(os.path.dirname(args.pretrained_weights), 'linear')).mkdir(parents=True, exist_ok=True)
