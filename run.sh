...
if [[ $TYPE =~ imagenet_knn ]] || [[ $TYPE =~ imagenet_reg ]] || \
   ...
   [[ $TYPE =~ davis_viseg ]] || [[ $TYPE =~ paris_oxford_reid ]] || [[ $TYPE =~ copydays_copydet ]]; then
    ...
    if [[ $TYPE =~ imagenet_cls ]] && [[ ! $TYPE =~ pretrain ]]; then
        ...
        for K in $(seq ${NODE_ID:-0} ${TOTAL_NODES:-1} $[${#KEY_LIST[@]}-1]) 
        do
            # ./run.sh imagenet_cls imnet_ft_vit_base vit_base teacher 1 --epochs 100 --drop_path 0.2 --layer_decay 0.65 --data_path ../../../../../../data/ImageNet2012 --model_key state_dict 
            
            # WEIGHT_FILE=$SUB_OUTPUT_DIR/checkpoint_${KEY_LIST[$K]}.pth
            # WEIGHT_FILE='../SOVE/runs/Aug08_22-07-44_nid005047/weights0399.pth'
            # SUB_OUTPUT_DIR='../SOVE/runs/Aug08_22-07-44_nid005047/cls/imnet_ft_weights_lr=2e-3'

            WEIGHT_FILE='../SOVE/runs/Oct01_17-22-22_nid007271/weights0249.pth'
            SUB_OUTPUT_DIR='../SOVE/runs/Oct01_17-22-22_nid007271/cls/imnet_ft_weights_lr=9e-4'

            # WEIGHT_FILE='/fp/projects01/ec35/homes/ec-thallesss/representation_learning/src/methods/pretrained-methods/ibot/vitb/weights_teacher.pth'
            # SUB_OUTPUT_DIR='/fp/projects01/ec35/homes/ec-thallesss/representation_learning/src/methods/pretrained-methods/ibot/vitb/finetune_ibot_base_2'
            ...
            python3 -m torch.distributed.launch --nproc_per_node=$GPUS_PER_NODE \
                --master_port=$[${MASTER_PORT:-295050}-$K] \
    ...
    if [[ $TYPE =~ cars_cls ]] && [[ ! $TYPE =~ pretrain ]]; then
        ...
        for K in $(seq ${NODE_ID:-0} ${TOTAL_NODES:-1} $[${#KEY_LIST[@]}-1]) 
        do
            python3 --nproc_per_node=$GPUS_PER_NODE \
                --master_port=$[${MASTER_PORT:-29500}-$K] \
                $CURDIR/evaluation/eval_cls.py --pretrained_weights $PRETRAINED \
                --avgpool_patchtokens $AVGPOOL \
                --arch $ARCH \
    ...
    if [[ $TYPE =~ ade20k_dense_linear ]] && [[ ! $TYPE =~ pretrain ]]; then
        ...
        do
            ...
            # fi
            WEIGHT_FILE='../NP_ibot/runs/Aug08_22-07-44_nid005047/weights0349.pth'
            SUB_OUTPUT_DIR='./runs/Aug08_22-07-44_nid005047/seg_linear'
            # python3 -m torch.distributed.launch --nproc_per_node=$GPUS_PER_NODE \
            #     --master_port=$[${MASTER_PORT:-29500}-$K] \
    ...
    if [[ $TYPE =~ ade20k_seg ]] && [[ ! $TYPE =~ pretrain ]]; then
        ...
        do
            ...
            #     python3 extract_backbone_weights.py $PRETRAINED $WEIGHT_FILE --checkpoint_key ${KEY_LIST[$K]}
            # fi
            WEIGHT_FILE='./runs/Aug08_22-07-44_nid005047/weights0349.pth'
            SUB_OUTPUT_DIR='./runs/Aug08_22-07-44_nid005047/seg'
    ...
    if [[ $TYPE =~ coco_det ]] && [[ ! $TYPE =~ pretrain ]]; then
        ...
        do
            ...
            # fi
            WEIGHT_FILE='./runs/Aug08_22-07-44_nid005047/weights0349.pth'
            SUB_OUTPUT_DIR='./runs/Aug08_22-07-44_nid005047/det'
            # python3 -m torch.distributed.launch --nproc_per_node=$GPUS_PER_NODE \
            #     --master_port=$[${MASTER_PORT:-29500}-$K] \
    ...
    if [[ $TYPE =~ davis_viseg ]] && [[ ! $TYPE =~ pretrain ]]; then
        ...
        do
            ...
            python3 \
                ...
                ${@:6}
            python3 \
                $CURDIR/davis2017-evaluation/evaluation_method.py \
